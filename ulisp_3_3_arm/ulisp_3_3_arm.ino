/*  uLisp ARM 3.3 - www.ulisp.com
    ...
    Kaef, 2020-06-05: added Teensy 4.1 support
*/
...
#include <SD.h>
// #include <SdFat.h> // Kaef tbd: needed Teensy 4?
#define SDSIZE 172
#else
#define SDSIZE 0
#endif
...
enum stream { SERIALSTREAM, I2CSTREAM, SPISTREAM, SDSTREAM, STRINEAM, GFXSTREAM };
...
float checkintfloat (symbol_t name, object *obj) {
    ...
    return 0.0; // Kaef: suppress compiler warning
}
...
object *sp_withspi (object *args, object *env) {
    ...
    #if defined(__IMXRT1062__) // TEENSY40 (Kaef)
...
object *fn_length (object *args, object *env) {
    ...
    return nil; // Kaef: suppress compiler warning
}
...
object *fn_makearray (object *args, object *env) {
    ...
    if (bitp) {
        if (def == nil) def = 0;    // 1 becomes all ones
        else def = number(-checkbitvalue(MAKEARRAY, def));
    }
...
EVAL:
    // Enough space?
#if defined(__IMXRT1062__) //TEENSY40 (Kaef)
#warning "Stack overflow check disabled!"
#else
...
void setup () {
    ...
    initsleep();
    initgfx();
    pfstring(PSTR("uLisp 3.3 (Teensy-4.1)"), pserial); pln(pserial);
}
